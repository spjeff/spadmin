...
test_userA1         test_userA1         test_userA1@lazy... test_managerA
		...
		foreach ($Account in $Identity)
		{
			TRY
			{
				IF ($PSBoundParameters['Recurse'])
				{
					...
					ForEach-Object -Process {
						$_.directreports | ForEach-Object -Process {
							...
							Get-ADUser -Identity $PSItem -Properties manager, employeeType, eMailAddress, Enabled, title | Select-Object -Property Name, SamAccountName, employeeType, Enabled, title, @{ Name = "Manager"; Expression = { (Get-Aduser -identity $psitem.manager).samaccountname } }
							$global:users+=Get-ADUser -Identity $PSItem -Properties manager, eMailAddress, employeeType, title, fmCostCenter | Select-Object -Property Name, SamAccountName, eMailAddress, employeeType, title, Enabled, fmCostCenter, @{ Name = "Manager"; Expression = { (Get-Aduser -identity $psitem.manager).samaccountname } }
							$global:i;$global:i++;
				...
				IF (-not ($PSBoundParameters['Recurse']))
				{
					...
					Get-ADUser -Properties manager, eMailAddress, employeeType, title, fmCostCenter | Select-Object -Property Name, SamAccountName, employeeType, eMailAddress, title, fmCostCenter, @{ Name = "Manager"; Expression = { (Get-Aduser -identity $psitem.manager).samaccountname } }
				}#IF (-not($PSBoundParameters['Recurse']))
			}#TRY
			CATCH
			{
	...
	$global:users.count
	write-host 'writing to c:\temp\DirectReports$(Get-Date -F "yyyyMMdd").csv'
	$global:users | export-csv c:\temp\DirectReports$(Get-Date -F "yyyyMMdd").csv -NoTypeInformation -Force
	}
	END
